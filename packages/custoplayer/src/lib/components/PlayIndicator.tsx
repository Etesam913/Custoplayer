...
function PlayIndicator() {
  ...
  const iconWidth =
    videoElem === null
      ? 160
      : clamp(
          (videoDimensions.width / videoElem.videoWidth) * 160 * 1.3,
          90,
          160,
        );
  const iconHeight =
    videoElem === null
      ? 160
      : clamp(
          (videoDimensions.height / videoElem.videoHeight) * 160 * 1.3,
          90,
          160,
        );
  ...
  return useMemo(() => {
    return (
      <AnimatePresence custom={isSeeking} mode='wait'>
        {isSeeking && (
          <IndicatorContainer
            custom={isSeeking}
...
const IndicatorContainer = styled(motion.svg)<{
